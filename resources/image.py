...
class ImageUpload(Resource):
    ...
    def post(cls):
        """
        This endpoint is used to upload an image file. 
        It uses the JWT to retrieve user information and save the image in the user's folder.
        ...
        unused integer. (eg. filename.png to filename_1.png). which is flask_uploads resolving naming conflict
    
        
        """
        # data is a dictionary of {"image": FileStorage} with it's value a file storage object from WERKZEUG
        # return would have been an ImageUpload object for flask_marshmallow
        data = image_schema.load(request.files)
        user_id = get_jwt_identity()
        folder = f"user_{user_id}"  # static/images/user_1 for example
        try:
            ...
            # using data["image"].filename) will not return filename_1.png when name conflicts unlike basename
            # return {"message": gettext("image_uploaded").format(data["image"].filename)}, 201
...
class AvatarUpload(Resource):
    ...
    def put(cls):
        ...
        It will overwrite the existing avatar if it exist or create new one if it does not exist.
        """
        data = image_schema.load(request.files)
